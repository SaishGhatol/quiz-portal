    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center p-8 max-w-md mx-auto bg-white rounded-xl shadow-md">
          ...
          <p className="text-lg font-medium text-gray-700">Loading quiz questions...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Question Form */}
        <div className="lg:col-span-5 bg-white rounded-xl shadow-md p-6 h-fit">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {(question.options || []).map((option, optIndex) => (
                            <div key={optIndex} className={`flex items-center p-2 rounded ${
                              ...
                              <span className={`w-5 h-5 flex-shrink-0 rounded-full flex items-center justify-center mr-2 ${
                                optIndex === question.correctOption 
          ...
          {questions && questions.length > 0 && (
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 text-blue-800 text-sm">
              ...
              <ul className="mt-2 pl-5 list-disc">
                ...
                <li>Total possible points: {questions.reduce((sum, q) => sum + (q.points || 1), 0)}</li>
                <li>Average points per question: {(questions.reduce((sum, q) => sum + (q.points || 1), 0) / questions.length).toFixed(1)}</li>
              </ul>
            </div>
          )}
        </div>
